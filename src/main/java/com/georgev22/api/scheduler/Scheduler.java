...
import com.georgev22.api.scheduler.interfaces.Task;
 ...
 * Adding to the tail is atomic and very efficient; utility method is {@link #handle(com.georgev22.api.scheduler.Task, long)} or {@link #addTask(com.georgev22.api.scheduler.Task)}. </li>
 ...
 */
public class Scheduler implements com.georgev22.api.scheduler.interfaces.Scheduler {
    ...
    /**
     * Current head of linked-list. This reference is always stale, {@link com.georgev22.api.scheduler.Task#next} is the live reference.
     */
    private volatile com.georgev22.api.scheduler.Task head = new com.georgev22.api.scheduler.Task();
    /**
     * Tail of a linked-list. AtomicReference only matters when adding to queue
     */
    private final AtomicReference<com.georgev22.api.scheduler.Task> tail = new AtomicReference<>(head);
    // If the tasks should run on the same tick they should be run FIFO
    /**
     * Main thread logic only
     */
    private final PriorityQueue<com.georgev22.api.scheduler.Task> pending = new PriorityQueue<>(10,
            Comparator.comparingLong(com.georgev22.api.scheduler.Task::getNextRun).thenComparingLong(com.georgev22.api.scheduler.Task::getCreatedAt));
    /**
     * Main thread logic only
     */
    private final List<com.georgev22.api.scheduler.Task> temp = new ArrayList<>();
    /**
     * These are tasks that are currently active. It's provided for 'viewing' the current state.
     */
    private final ConcurrentHashMap<Integer, com.georgev22.api.scheduler.Task> runners = new ConcurrentHashMap<>();
    /**
     * The sync task that is currently running on the main thread.
     */
    private volatile com.georgev22.api.scheduler.Task currentTask = null;
    ...
    public int scheduleSyncDelayedTask(final @NotNull Class<?> clazz, final @NotNull Runnable task, final long delay) {
        return this.scheduleSyncRepeatingTask(clazz, task, delay, com.georgev22.api.scheduler.Task.NO_REPEATING);
    }

    @Override
    public @NotNull Task runTaskLater(@NotNull Class<?> clazz, @NotNull Runnable runnable, long delay) {
        return runTaskTimer(clazz, runnable, delay, com.georgev22.api.scheduler.Task.NO_REPEATING);
    }

    @Override
    public void runTaskLater(@NotNull Class<?> clazz, @NotNull Consumer<Task> task, long delay) throws IllegalArgumentException {
        runTaskTimer(clazz, task, delay, com.georgev22.api.scheduler.Task.NO_REPEATING);
    }

    @Deprecated
    @Override
    public int scheduleAsyncDelayedTask(final @NotNull Class<?> clazz, final @NotNull Runnable task, final long delay) {
        return this.scheduleAsyncRepeatingTask(clazz, task, delay, com.georgev22.api.scheduler.Task.NO_REPEATING);
    }

    @Override
    public @NotNull Task runTaskLaterAsynchronously(@NotNull Class<?> clazz, @NotNull Runnable runnable, long delay) {
        return runTaskTimerAsynchronously(clazz, runnable, delay, com.georgev22.api.scheduler.Task.NO_REPEATING);
    }
    ...
    public void runTaskLaterAsynchronously(@NotNull Class<?> clazz, @NotNull Consumer<Task> task, long delay) throws IllegalArgumentException {
        runTaskTimerAsynchronously(clazz, task, delay, com.georgev22.api.scheduler.Task.NO_REPEATING);
    }
    ...
    public void runTaskTimerAsynchronously(@NotNull Class<?> clazz, @NotNull Consumer<Task> task, long delay, long period) throws IllegalArgumentException {
        runTaskTimerAsynchronously(clazz, (Object) task, delay, com.georgev22.api.scheduler.Task.NO_REPEATING);
    }
    ...
    public Task runTaskTimer(Class<?> clazz, Object runnable, long delay, long period) {
        ...
        if (period == com.georgev22.api.scheduler.Task.ERROR) {
            period = 1L;
        } else if (period < com.georgev22.api.scheduler.Task.NO_REPEATING) {
            period = com.georgev22.api.scheduler.Task.NO_REPEATING;
        }
        return handle(new com.georgev22.api.scheduler.Task(clazz, runnable, nextId(), period), delay);
    }
    ...
    public void cancelTask(final int taskId) {
        ...
        com.georgev22.api.scheduler.Task task = runners.get(taskId);
        if (task != null) {
            task.cancel0();
        }
        task = new com.georgev22.api.scheduler.Task(
                new Runnable() {
                    @Override
                    public void run() {
                    ...
                    private boolean check(final Iterable<com.georgev22.api.scheduler.Task> collection) {
                        final Iterator<com.georgev22.api.scheduler.Task> tasks = collection.iterator();
                        while (tasks.hasNext()) {
                            final com.georgev22.api.scheduler.Task task = tasks.next();
        ...
        handle(task, 0L);
        for (com.georgev22.api.scheduler.Task taskPending = head.getNext(); taskPending != null; taskPending = taskPending.getNext()) {
            if (taskPending == task) {
                return;
            }
    ...
    public void cancelTasks(final @NotNull Class<?> clazz) {
        ...
        final com.georgev22.api.scheduler.Task task = new com.georgev22.api.scheduler.Task(
                new Runnable() {
                    @Override
                    public void run() {
                        check(Scheduler.this.pending);
                    ...
                    void check(final Iterable<com.georgev22.api.scheduler.Task> collection) {
                        final Iterator<com.georgev22.api.scheduler.Task> tasks = collection.iterator();
                        while (tasks.hasNext()) {
                            final com.georgev22.api.scheduler.Task task = tasks.next();
        ...
        for (com.georgev22.api.scheduler.Task runner : runners.values()) {
    ...
    public boolean isCurrentlyRunning(final int taskId) {
        final com.georgev22.api.scheduler.Task task = runners.get(taskId);
        if (task == null) {
            return false;
        }
        if (task.isSync()) {
    ...
    public boolean isQueued(final int taskId) {
        ...
        for (com.georgev22.api.scheduler.Task task = head.getNext(); task != null; task = task.getNext()) {
            if (task.getTaskId() == taskId) {
                return task.getPeriod() >= com.georgev22.api.scheduler.Task.NO_REPEATING; // The task will run
            }
        }
        com.georgev22.api.scheduler.Task task = runners.get(taskId);
        return task != null && task.getPeriod() >= com.georgev22.api.scheduler.Task.NO_REPEATING;
    }

    @Override
    public @NotNull List<Worker> getActiveWorkers() {
        final ArrayList<Worker> workers = new ArrayList<>();
        for (final com.georgev22.api.scheduler.Task taskObj : runners.values()) {
    ...
    public @NotNull List<Task> getPendingTasks() {
        final ArrayList<com.georgev22.api.scheduler.Task> truePending = new ArrayList<>();
        for (com.georgev22.api.scheduler.Task task = head.getNext(); task != null; task = task.getNext()) {
            if (task.getTaskId() != -1) {
                // -1 is special code
        ...
        for (com.georgev22.api.scheduler.Task task : runners.values()) {
            if (task.getPeriod() >= com.georgev22.api.scheduler.Task.NO_REPEATING) {
                pending.add(task);
            }
        }

        for (final com.georgev22.api.scheduler.Task task : truePending) {
            if (task.getPeriod() >= com.georgev22.api.scheduler.Task.NO_REPEATING && !pending.contains(task)) {
                pending.add(task);
            }
        }
        return pending;
    ...
    public void mainThreadHeartbeat(final int currentTick) {
        ...
        final List<com.georgev22.api.scheduler.Task> temp = this.temp;
        parsePending();
        while (isReady(currentTick)) {
            final com.georgev22.api.scheduler.Task task = pending.remove();
            if (task.getPeriod() < com.georgev22.api.scheduler.Task.NO_REPEATING) {
                if (task.isSync()) {
    ...
    private void addTask(final com.georgev22.api.scheduler.Task task) {
        final AtomicReference<com.georgev22.api.scheduler.Task> tail = this.tail;
        com.georgev22.api.scheduler.Task tailTask = tail.get();
    ...
    private com.georgev22.api.scheduler.Task handle(final com.georgev22.api.scheduler.Task task, final long delay) {
    ...
    private void parsePending() {
        com.georgev22.api.scheduler.Task head = this.head;
        com.georgev22.api.scheduler.Task task = head.getNext();
        com.georgev22.api.scheduler.Task lastTask = head;
        for (; task != null; task = (lastTask = task).getNext()) {
            if (task.getTaskId() == -1) {
                task.run();
            } else if (task.getPeriod() >= com.georgev22.api.scheduler.Task.NO_REPEATING) {
                pending.add(task);
    ...
    @Deprecated
    @Override
    public @NotNull com.georgev22.api.scheduler.Task runTask(@NotNull Class<?> clazz, @NotNull SchedulerRunnable task) throws IllegalArgumentException {
        throw new UnsupportedOperationException("Use SchedulerRunnable#runTask(Class)");
    }

    @Deprecated
    @Override
    public @NotNull com.georgev22.api.scheduler.Task runTaskAsynchronously(@NotNull Class<?> clazz, @NotNull SchedulerRunnable task) throws IllegalArgumentException {
        throw new UnsupportedOperationException("Use SchedulerRunnable#runTaskAsynchronously(Class)");
    }

    @Deprecated
    @Override
    public @NotNull com.georgev22.api.scheduler.Task runTaskLater(@NotNull Class<?> clazz, @NotNull SchedulerRunnable task, long delay) throws IllegalArgumentException {
        throw new UnsupportedOperationException("Use SchedulerRunnable#runTaskLater(Class, long)");
    }

    @Deprecated
    @Override
    public @NotNull com.georgev22.api.scheduler.Task runTaskLaterAsynchronously(@NotNull Class<?> clazz, @NotNull SchedulerRunnable task, long delay) throws IllegalArgumentException {
        throw new UnsupportedOperationException("Use SchedulerRunnable#runTaskLaterAsynchronously(Class, long)");
    }

    @Deprecated
    @Override
    public @NotNull com.georgev22.api.scheduler.Task runTaskTimer(@NotNull Class<?> clazz, @NotNull SchedulerRunnable task, long delay, long period) throws IllegalArgumentException {
        throw new UnsupportedOperationException("Use SchedulerRunnable#runTaskTimer(Class, long, long)");
    }

    @Deprecated
    @Override
    public @NotNull com.georgev22.api.scheduler.Task runTaskTimerAsynchronously(@NotNull Class<?> clazz, @NotNull SchedulerRunnable task, long delay, long period) throws IllegalArgumentException {
